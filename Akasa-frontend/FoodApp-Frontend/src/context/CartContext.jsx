    ...
    useEffect(() => {
        const updateCartTotals = () => {
            const subtotal = cartData.items.reduce((acc, cur) => {
                const itemSubtotal = parseFloat(cur.subtotal) || 0;  
                console.log(`Item Subtotal: ${itemSubtotal}, Accumulated: ${acc}`); 
                return acc + itemSubtotal;
            }, 0);

            const totalQuantity = cartData.items.reduce((acc, cur) => {
                const itemQuantity = parseInt(cur.quantity) || 0;  
                console.log(`Item Quantity: ${itemQuantity}, Accumulated: ${acc}`);  
