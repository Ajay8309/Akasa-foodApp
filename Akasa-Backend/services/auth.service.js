...
class AuthService {
    ...
    async signUp(user) {
        try {
            ...
            if(validateUser(email, password)) {
                ...
                const { cart_id } = await createCartDb(newUser.user_id);
                console.log(cart_id);
                const { wishlist_id } = await createWishlistDb(newUser.user_id);
    ...
    async login(email, password) {
        ...
        //   console.log("Received login request with email:", email);
          ...
          const token = await this.signToken({ id: user_id, cart_id, wishlist_id });
          const refreshToken = await this.signRefreshToken({
            id: user_id,
            // roles,
            cart_id,
